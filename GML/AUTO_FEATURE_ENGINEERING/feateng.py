...
def engineer_features(
    ...
    if not start_features:
        ...
        for c in start_features:
            if c not in df_org.columns:
                raise ValueError("[FEATURE_ENGINEERING] start feature %r not in df_org.columns" % c)
    feature_pool = {c: sympy.symbols(colnames2symbols(c, i), real=True) for i, c in enumerate(start_features)}
    if max_steps < 1:
        if verbose > 0:
            print("[FEATURE_ENGINEERING] Warning: no features generated for max_steps < 1.")
    ...
    def apply_transformations(features_list):
        ...
        for i, feat in enumerate(features_list):
            if verbose and not i % 100:
                print("[FEATURE_ENGINEERING] %15i/%15i features transformed" % (i, len(features_list)), end="\r")
        ...
        if verbose > 0:
            print("[FEATURE_ENGINEERING] Generated %i transformed features from %i original features - done." % (len(new_features), len(features_list)))
    ...
    def get_feature_combinations(feature_tuples):
        ...
        for i, (feat1, feat2) in enumerate(feature_tuples):
            if verbose and not i % 100:
                print("[FEATURE_ENGINEERING] %15i/%15i feature tuples combined" % (i, len(feature_tuples)), end="\r")
                    ...
                    if np.isfinite(new_feat).all() and np.var(new_feat) > 1e-10:
                        ...
                        if corr < 1.:
                            ...
                            if corr < 0.95:
                                uncorr_features.add(expr_name)
        if verbose > 0:
            print("[FEATURE_ENGINEERING] Generated %i feature combinations from %i original feature tuples - done." % (len(new_features), len(feature_tuples)))
        df = df.join(pd.DataFrame(feat_array[:, :len(new_features)], columns=new_features, index=df.index, dtype=np.float32))
        return new_features, uncorr_features
    ...
    steps = 1
    if verbose > 0:
        print("[FEATURE_ENGINEERING] Step 1: transformation of original features")
    ...
    if steps <= max_steps:
        if verbose > 0:
            print("[FEATURE_ENGINEERING] Step 2: first combination of features")
    ...
    while steps <= max_steps:
        # apply transformations on these new features
        if verbose > 0:
            print("[FEATURE_ENGINEERING] Step %i: transformation of new features" % steps)
        ...
        if steps <= max_steps:
            if verbose > 0:
                print("[FEATURE_ENGINEERING] Step %i: combining old and new features" % steps)
        ...
        if steps <= max_steps:
            if verbose > 0:
                print("[FEATURE_ENGINEERING] Step %i: combining new features" % steps)
    ...
    if verbose > 0:
        print("[FEATURE_ENGINEERING] Generated altogether %i new features in %i steps" % (len(feature_pool) - len(start_features), max_steps))
        print("[FEATURE_ENGINEERING] Removing correlated features, as well as additions at the highest level")
    ...
    if verbose > 0:
        print("[FEATURE_ENGINEERING] Generated a total of %i additional features" % (len(feature_pool) - len(start_features)))

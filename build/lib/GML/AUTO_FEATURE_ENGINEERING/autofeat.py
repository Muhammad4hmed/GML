    ...
    if units:
        ...
        for c in units:
            try:
                ...
                if verbose > 0:
                    print("[GML] WARNING: unit %r of column %r was not recognized and will be ignored!" % (units[c], c))
...
class AutoFeatModel(BaseEstimator):
        ...
        if self.categorical_cols:
            ...
            for c in self.categorical_cols:
                if c not in df.columns:
                    raise ValueError("[GML] categorical_col %r not in df.columns" % c)
    ...
    def _generate_features(self, df, new_feat_cols):
        ...
        if not new_feat_cols[0] in self.feature_formulas_:
            raise RuntimeError("[GML] First call fit or fit_transform to generate the features!")
        if self.verbose:
            print("[GML] Computing %i new features." % len(new_feat_cols))
        ...
        for i, expr in enumerate(new_feat_cols):
            if self.verbose:
                print("[GML] %5i/%5i new features" % (i, len(new_feat_cols)), end="\r")
            if expr not in self.feature_functions_:
                ...
                if not cols:
                    ...
                    try:
                        ...
                        print("[GML] Error while processing expression: %r" % expr)
                        raise
            ...
            if f is not None:
                ...
                try:
                    ...
                    print("[GML] WARNING: Problem while evaluating expression: %r with columns %r" % (expr, cols),
                          " - is the data in a different range then when calling .fit()? Are maybe some values 0 that shouldn't be?")
                    raise
        if self.verbose:
            print("[GML] %5i/%5i new features ...done." % (len(new_feat_cols), len(new_feat_cols)))
    ...
    def fit_transform(self, X, y):
        ...
        if self.feateng_cols:
            ...
            for c in self.feateng_cols:
                if c not in self.original_columns_:
                    raise ValueError("[GML] feateng_col %r not in df.columns" % c)
        ...
        if self.verbose:
            print("[GML] The %i step feature engineering process could generate up to %i features." % (self.feateng_steps, n_cols))
            print("[GML] With %i data points this new feature matrix would use about %.2f gb of space." % (len(df), n_gb))
        if self.max_gb and n_gb > self.max_gb:
            n_rows = int(self.max_gb * 250000000 / n_cols)
            if self.verbose:
                print("[GML] As you specified a limit of %.1d gb, the number of data points is subsampled to %i" % (self.max_gb, n_rows))
        ...
        if self.featsel_runs <= 0:
            if self.verbose:
                print("[GML] WARNING: Not performing feature selection.")
            ...
            if self.problem_type in ("regression", "classification"):
                ...
                print("[GML] WARNING: Unknown problem_type %r - not performing feature selection." % self.problem_type)
                good_cols = df_subs.columns
        ...
        if self.verbose:
            ...
            print("[GML] Final dataframe with %i feature columns (%i new)." % (len(df.columns), len(df.columns) - len(self.original_columns_)))
        ...
        if self.problem_type == "regression":
            ...
            print("[GML] WARNING: Unknown problem_type %r - not fitting a prediction model." % self.problem_type)
            model = None
        if model is not None:
            if self.verbose:
                print("[GML] Training final %s model." % self.problem_type)
    ...
    def fit(self, X, y):
        if self.verbose:
            print("[GML] Warning: This just calls fit_transform() but does not return the transformed dataframe.")
            print("[GML] It is much more efficient to call fit_transform() instead of fit() and transform()!")
    ...
    def transform(self, X):
        ...
        if not cols == self.original_columns_:
            raise ValueError("[GML] Not the same features as when calling fit.")
        # transform X into a dataframe (again)
        df = pd.DataFrame(X, columns=cols)
        # possibly convert categorical columns
        df = self._transform_categorical_cols(df)

...
def _noise_filtering(X, target, good_cols=[], problem_type="regression"):
    ...
    if problem_type == "regression":
        ...
        print("[FEATURE_SELECTION] WARNING: Unknown problem_type %r - not performing noise filtering." % problem_type)
        model = None
...
def _select_features_1run(df, target, problem_type="regression", verbose=0):
    ...
    if problem_type == "regression":
        ...
        print("[FEATURE_SELECTION] WARNING: Unknown problem_type %r - not performing feature selection!" % problem_type)
        return []
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        # TODO: remove if sklearn least_angle issue is fixed
    ...
    if verbose > 0:
        print("[FEATURE_SELECTION]\t %i initial features." % len(initial_cols))
    # add noise features
    ...
    if other_cols:
        ...
        for i in range(n_splits):
            ...
            if verbose > 0:
                print("[FEATURE_SELECTION]\t Split %2i/%i: %3i candidate features identified." % (i+1, n_splits, len(good_cols)), end="\r")
    ...
    if verbose > 0:
        print("\n[FEATURE_SELECTION]\t Selected %3i features after noise filtering." % len(good_cols))
    return good_cols
...
def select_features(df, target, featsel_runs=5, keep=None, problem_type="regression", n_jobs=1, verbose=0):
    ...
    if not (len(df) == len(target)):
        raise ValueError("[FEATURE_SELECTION] df and target dimension mismatch.")
    if keep is None:
        keep = []
    ...
    if verbose > 0:
        if featsel_runs > df.shape[0]:
            print("[FEATURE_SELECTION] WARNING: Less data points than featsel runs!!")
        print("[FEATURE_SELECTION] Scaling data...", end="")
    ...
    def run_select_features(i):
        if verbose > 0:
            print("[FEATURE_SELECTION] Feature selection run %i/%i" % (i+1, featsel_runs))
        np.random.seed(i)
    ...
    if featsel_runs >= 1 and problem_type in ("regression", "classification"):
        ...
        if selected_columns:
            ...
            if verbose > 0:
                print("[FEATURE_SELECTION] %i features after %i feature selection runs" % (len(selected_columns), featsel_runs))
            ...
            if len(selected_columns) > k:
                ...
                for i, c in enumerate(selected_columns[k:], k):
                    ...
                    if np.max(np.abs(correlations[c].ravel()[:i])) < 0.9:
                        good_cols.append(c)
            if verbose > 0:
                print("[FEATURE_SELECTION] %i features after correlation filtering" % len(good_cols))

    # perform noise filtering on these features
    good_cols = _noise_filtering(df_scaled[good_cols].to_numpy(), target_scaled, good_cols, problem_type)
    if verbose > 0:
        print("[FEATURE_SELECTION] %i features after noise filtering" % len(good_cols))
        if not good_cols:
            print("[FEATURE_SELECTION] WARNING: Not a single good features was found...")

    # add keep columns back in
    good_cols = keep + [c for c in good_cols if c not in keep]
    if verbose > 0 and keep:
        print("[FEATURE_SELECTION] %i final features selected (including %i original keep features)." % (len(good_cols), len(keep)))
    return good_cols
